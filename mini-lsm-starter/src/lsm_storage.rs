...
impl LsmStorageInner {
    ...
    pub fn get(&self, _key: &[u8]) -> Result<Option<Bytes>> {
        ...
        let value_op = guard.memtable.get(_key);
        let result_predicate = |v: &Bytes| !v.is_empty();
        if value_op.is_some() {
            return Ok(value_op.filter(result_predicate));
        }
        for imm_table in guard.imm_memtables.iter() {
            let val_op = imm_table.get(_key);
            if val_op.is_some() {
                return Ok(val_op.filter(result_predicate));
            }
        }
    ...
    fn try_to_freeze(&self, guard: RwLockReadGuard<RawRwLock, Arc<LsmStorageState>>) {
        if self.is_memtable_limit_reached() {
    ...
    pub fn scan(
        ...
        let mut vector_with_iters = vec![];
        let memtable_iter = read_guard.memtable.scan(_lower, _upper);
        vector_with_iters.push(Box::new(memtable_iter));
        read_guard.imm_memtables.iter().for_each(|memtable| {
            let iter = memtable.scan(_lower, _upper);
            vector_with_iters.push(Box::new(iter));
        });
        let merge_iterator = MergeIterator::create(vector_with_iters);
        let result = FusedIterator::new(LsmIterator::new(merge_iterator).unwrap());
